...
export function HomePanels({ skinCount, mapCount }: { skinCount: number; mapCount: number }) {
  const t = useT();
    ...
    <div className="flex flex-col gap-[6px]">
      ...
      <Panel>
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 border-b border-[#22333d] bg-[#101c23] px-3 py-2">
          ...
          <span className="text-[12px] text-cs-text/90">{t("home.dailyWaiting")}</span>
          <span className="ml-auto text-[10px] uppercase tracking-wide text-cs-dim2">{t("home.resets")}</span>
        </div>
      ...
      <Panel>
        <PanelHead
          ...
          title={t("home.briefing")}
          right={<span className="text-[10px] uppercase tracking-wide">{t("nav.settings")}</span>}
        />
        <HomeHero skinCount={skinCount} mapCount={mapCount} />
      </Panel>
    </div>
  );
}
